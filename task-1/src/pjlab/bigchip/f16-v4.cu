...
__global__ void matmul_f16_v4(const float16_t* A, const float16_t* B, float16_t* C,
                          int M, int N, int K) {
...
PLAYGROUND_MATMUL_SIG(float16_t, 4, M, N, K, A, B, C) {
    ...
    cudaFuncSetAttribute(matmul_f16_v4,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);

    unsigned int dsmem = 2 * (BM * (BK + 8) + BK * (BN + 8)) * sizeof(half);
    matmul_f16_v4<<<gridDim, blockDim, dsmem>>>(A, B, C, M, N, K);
