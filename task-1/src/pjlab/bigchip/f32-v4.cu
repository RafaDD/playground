...
__global__ void matmul_f32_v4(const float32_t* a, const float32_t* b, float32_t* c, int M,
                              int N, int K)
{
    const int BM = 128;
    const int BN = 128;
    ...
    __shared__ __align__(16 * 1024) float s_a[BK][BM];
...
#pragma unroll
    ...
    // cut into 4 pieces and compute respectively
#pragma unroll
...
#pragma unroll
    ...
    PLAYGROUND_MATMUL_SIG(float32_t, 4, M, N, K, A, B, C)
    {
        ...
        matmul_f32_v4<<<GridSize, BlockSize>>>(A, B, C, M, N, K);
    }
