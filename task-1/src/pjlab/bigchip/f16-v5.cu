...
__global__ void matmul_f16_v5(const float16_t* A, const float16_t* B, float16_t* C,
                          int M, int N, int K) {
...
#pragma unroll
    ...
    {
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(load_a_smem_addr_0 + s_a_db_offset * (int) sizeof(half)), "l"(&A[load_a_gmem_addr]));
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(load_a_smem_addr_1 + s_a_db_offset * (int) sizeof(half)), "l"(&A[load_a_gmem_addr + K]));
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(load_b_smem_addr_0 + s_b_db_offset * (int) sizeof(half)), "l"(&B[load_b_gmem_addr]));
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(load_b_smem_addr_1 + s_b_db_offset * (int) sizeof(half)), "l"(&B[load_b_gmem_addr + N]));
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(load_b_smem_addr_2 + s_b_db_offset * (int) sizeof(half)), "l"(&B[load_b_gmem_addr + 2 * N]));
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(load_b_smem_addr_3 + s_b_db_offset * (int) sizeof(half)), "l"(&B[load_b_gmem_addr + 3 * N]));
...
#pragma unroll 32
    for (int bk = 0; bk < K / BK - 2; bk++) {

        int smem_sel = bk % 3;
        int smem_sel_next = (bk + 2) % 3;
...
PLAYGROUND_MATMUL_SIG(float16_t, 5, M, N, K, A, B, C) {
    ...
    cudaFuncSetAttribute(matmul_f16_v5,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, 108304);

    unsigned int dsmem = 3 * (BM * (BK + 8) + BK * (BN + 8)) * sizeof(half);
    matmul_f16_v5<<<gridDim, blockDim, dsmem>>>(A, B, C, M, N, K);
